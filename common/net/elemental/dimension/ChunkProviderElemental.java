...
public class ChunkProviderElemental implements IChunkProvider {
	...
	public ChunkProviderElemental(World world, long seed, boolean mapFeaturesEnabled) {
		this.worldObj = world;
					...
					for (int l1 = 0; l1 < 8; ++l1) {
						...
						for (int i2 = 0; i2 < 4; ++i2) {
							...
							for (int k2 = 0; k2 < 4; ++k2) {
								if ((d16 += d15) > 0.0D) {
									par3ArrayOfByte[j2 += short1] = (byte) Biomes.MAIN_FILLER_BLOCK;
								} else if (k1 * 8 + l1 < b2) {
	...
	public void replaceBlocksForBiome(int par1, int par2, byte[] par3ArrayOfByte, BiomeGenBase[] par4ArrayOfBiomeGenBase) {
		...
		for (int k = 0; k < 16; ++k) {
			for (int l = 0; l < 16; ++l) {
				BasicElementalBiomeGen biomegenbase = (BasicElementalBiomeGen) par4ArrayOfBiomeGenBase[l + k * 16];
					...
					if (k1 <= 0 + this.rand.nextInt(5)) {
						...
						if (b3 == 0) {
							...
							if (j1 == -1) {
								...
								if (k1 < b0 && b1 == 0) {
									if (f < 0.15F) {
										b1 = biomegenbase.getFrozenBlock();
									} else {
										b1 = biomegenbase.getLiquidBlock();
									}
								}
